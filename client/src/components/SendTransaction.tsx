...
const SendTransaction: React.FC = () => {
  ...
  // Modified handleDispatch to accept a callback
  const handleDispatch = useCallback(() => {
    dispatch({
      type: Actions.SendTransaction,
      payload: {
        onSuccess: (transactionHash: string) => {
          // Redirect upon successful transaction
          history(`/transaction/${transactionHash}`);
        }
      },
    });
  }, [dispatch, history]); // Include history in dependency array
  ...
  return (
      <>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            <div
                className="hs-overlay-open:opacity-100 hs-overlay-open:duration-500 opacity-100 transition-all w-full m-3 mx-auto flex flex-col h-full items-center justify-center">
